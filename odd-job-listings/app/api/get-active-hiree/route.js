/**
 * This is for getting all the jobs that the hiree has booked (sheet 3)
 * All jobs for which USERNAME is the hiree 
 */
...
export async function GET(req) {
  try {
    ...
    const hireeIndex = headers.indexOf("hiree"); // 🔹 Adjust if needed

    if (hireeIndex === -1) {
      return Response.json({ error: "hiree column not found" }, { status: 400 });
    }

    // Filter rows where username matches
    const filteredRows = rows.slice(1).filter((row) => row[hireeIndex] === username);
    ...
    console.error('Error in /api/get-active-hiree:', error);
